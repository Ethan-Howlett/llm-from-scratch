...
class C4StreamDataset(IterableDataset):
    """Streams C4 (or any HF dataset with 'text' field), tokenizes and yields (input_ids, target_ids) chunks."""

    def __init__(self, stream, tokenizer, max_length: int, stride: int):
        self.stream = stream
    ...
    def __iter__(self):
        ...
        for example in self.stream:
            ...
            while len(buffer) >= self.max_length + 1:
                input_chunk = buffer[: self.max_length]
                target_chunk = buffer[1 : self.max_length + 1]
                buffer = buffer[self.stride :]
                yield torch.tensor(input_chunk), torch.tensor(target_chunk)


def create_c4_dataloader(stream, tokenizer, max_length: int, stride: int, batch_size: int, drop_last: bool = True):
    """Build a DataLoader over C4 streaming dataset."""
    dataset = C4StreamDataset(stream, tokenizer, max_length, stride)
    return DataLoader(
        dataset,
...
def calc_loss_loader_streaming(data_loader, model, device, num_batches: int):
    """Compute average loss over num_batches from an iterable loader (no len())."""
    total_loss = 0.0
    count = 0
...
def evaluate_model_streaming(model, train_loss_last, val_loader, device, eval_iter: int):
    """Eval for streaming: use last train loss, compute val loss over eval_iter batches."""
    model.eval()
    with torch.no_grad():
        val_loss = calc_loss_loader_streaming(val_loader, model, device, num_batches=eval_iter)
    model.train()
    return train_loss_last, val_loss
...
def train_model_streaming(
    ...
    """Step-based training for streaming dataloaders (no epochs)."""
    ...
    for _ in range(max_steps):
        ...
        if global_step % eval_freq == 0:
            train_loss = sum(recent_losses) / len(recent_losses) if recent_losses else float("nan")
            train_loss, val_loss = evaluate_model_streaming(
                model, train_loss, val_loader, device, eval_iter
            )
...
def main(gpt_config, settings):
    ...
    ##############################
    # Data source: "text" (local file) or "c4" (streaming)
    ##############################
    data_source = settings.get("data_source", "text")
    tokenizer = tiktoken.get_encoding("gpt2")
    print(f"Data source: {data_source}")

    if data_source == "text":
        file_path = "sample_texts/middlemarch.txt"
    ...
    if data_source == "text":
        ...
        max_steps = settings.get("max_steps", 10_000)
        train_losses, val_losses, tokens_seen = train_model_streaming(
            model, train_loader, val_loader, optimizer, device,
            max_steps=max_steps, eval_freq=10, eval_iter=1,
...
if __name__ == "__main__":
    ...
    OTHER_SETTINGS = {
        ...
        "batch_size": 32,
        "weight_decay": 0.1,
        "data_source": "text",   # "text" = local file (middlemarch.txt); "c4" = streaming C4
        # "max_steps": 10_000,    # only for data_source="c4"
    }
    ...
    # Plot results (eval index on x-axis for both text and C4)
